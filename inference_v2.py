...
def inference(img, encoder, decoder, generator, knn, least_square, device):
    ...
    pos = [(186, 244), (326, 244), (256, 302), (256, 385), (256, 256)]
    sz  = [128, 128, 160, 192, 512]
    img_part = [
        img[:, :, pos[0][1]-sz[0]//2:pos[0][1]+sz[0]//2, pos[0][0]-sz[0]//2:pos[0][0]+sz[0]//2].clone(),
        img[:, :, pos[1][1]-sz[1]//2:pos[1][1]+sz[1]//2, pos[1][0]-sz[1]//2:pos[1][0]+sz[1]//2].clone(),
        img[:, :, pos[2][1]-sz[2]//2:pos[2][1]+sz[2]//2, pos[2][0]-sz[2]//2:pos[2][0]+sz[2]//2].clone(),
        img[:, :, pos[3][1]-sz[3]//2:pos[3][1]+sz[3]//2, pos[3][0]-sz[3]//2:pos[3][0]+sz[3]//2].clone(),
        img[:, :, pos[4][1]-sz[4]//2:pos[4][1]+sz[4]//2, pos[4][0]-sz[4]//2:pos[4][0]+sz[4]//2].clone(),
    ]
    for i in range(4): img_part[4][:, :, pos[i][1]-sz[i]//2:pos[i][1]+sz[i]//2, pos[i][0]-sz[i]//2:pos[i][0]+sz[i]//2] = 0

    fv = [encoder[i](img_part[i]).to(device) for i in range(5)]
    def fv_proj(fv, idx, t = 0.2):
    ...
    for i in range(4, -1, -1):
        sketch[:, 0:1,
                      ...
                      pos[i][0]-sz[i]//2:pos[i][0]+sz[i]//2] = decoder[i](fv[i])
