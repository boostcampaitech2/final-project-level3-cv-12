...
def train(encoder, decoder, args):
    ...
    train_dataset = CEDataset('/opt/ml/project/data/train.json', args.part,
                              transform=GaussNoise(var_limit=(0, 0.2), mean=0.1, per_channel=True, always_apply=False, p=0.5))
    ...
    for epoch in range(args.epoch):
        ...
        with torch.no_grad():
            ...
            for step, (t, x) in enumerate(val_loader):
                ...
                if step % 10 == 0:
                    ...
                    if epoch == 0:
                        ...
                        img = (np.array(t.detach().cpu()).squeeze(
                            axis=1)[0]*255).astype('uint8')
                        # cv2.imwrite(path+f'original_{step}.png', img)

                    if epoch % 10 == 9:
                        img = (np.array(y.detach().cpu()).squeeze(
                            axis=1)[0]*255).astype('uint8')
            ...
            print(
                f'Validation #{epoch+1} Average Loss : {round(avg.item(),6)}')
            if epoch % 10 == 9:
                save_model(encoder, saved_dir=args.save_dir,
                           file_name=f'encoder_{args.part}_{epoch}_{int(avg.item() * 100000)}')
                save_model(decoder, saved_dir=args.save_dir,
                           file_name=f'decoder_{args.part}_{epoch}_{int(avg.item() * 100000)}')
        run.log({'table_key': test_table})


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--part', type=str, default='nose')
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument('--save_dir', type=str,
                        default='/opt/ml/project/save_pth')
    parser.add_argument('--sample_img', type=str,
                        default='/opt/ml/project/save_pth/sample_img')
    parser.add_argument('--epoch', type=int, default=200)
    parser.add_argument('--batch_size', type=int, default=8)
    args = parser.parse_args()

    part_encoder = CEModule.define_part_encoder(
        model=args.part, norm='instance', input_nc=1, latent_dim=512)
    part_decoder = CEModule.define_part_decoder(
        model=args.part, norm='instance', output_nc=1, latent_dim=512)
