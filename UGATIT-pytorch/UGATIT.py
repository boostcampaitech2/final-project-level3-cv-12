...
class UGATIT(object) :
    ...
    def build_model(self):
        ...
        train_transform_sketch = transforms.Compose([
            transforms.RandomHorizontalFlip(),
            transforms.Resize((self.img_size , self.img_size)),
            # transforms.RandomCrop(self.img_size),
        ...
        self.trainA = ImageFolder(os.path.join('dataset', self.dataset, 'trainA'), train_transform_sketch)
    ...
    def train(self):
        ...
        run = wandb.init(project = 'U-GAT-IT',entity='bcaitech_cv2')
        ...
        if self.resume:
            ...
            if not len(model_list) == 0:
                ...
                self.load(os.path.join(self.result_dir, self.dataset, 'model'), 108000)
                print(" [*] Load SUCCESS")
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            wandb.log({'Train/d_loss':Discriminator_loss})
            wandb.log({'Train/g_loss':Generator_loss})
            if step % 100== 0:
                t = time.strftime("%H:%M:%S")
                print(t+" - [%5d/%5d] time: %4.4f d_loss: %.8f, g_loss: %.8f" % (step, self.iteration, time.time() - start_time, Discriminator_loss, Generator_loss))
    ...
    def inference(self):
        model_list = glob(os.path.join(self.result_dir, self.dataset, 'model', '*.pt'))
        if not len(model_list) == 0:
            if self.iter_for_inference == 0:
                model_list.sort()
                iter = int(model_list[-1].split('_')[-1].split('.')[0])
            else:
                iter = self.iter_for_inference
            self.load(os.path.join(self.result_dir, self.dataset, 'model'), iter)
            print(" [*] Load SUCCESS")
        else:
            print(" [*] Load FAILURE")
